...
#define SHIFT_BITS (((CAPSENSE_KEYMAP_COL_TO_GPIO_COL(MATRIX_COLS - 1) >= 16) || \
                     (CAPSENSE_KEYMAP_COL_TO_GPIO_COL(0) >= 16)) ? 24 : 16)
...
void test_v2(void) {
    ...
    for (d=90;d<=260;d++)
    {
        ...
        dac_write_threshold(d);
            ...
            {
                test_col_print_data_v2(CAPSENSE_KEYMAP_COL_TO_GPIO_COL(c));
            }
        #else
...
{
    return measure_middle(CAPSENSE_KEYMAP_COL_TO_GPIO_COL(col), CAPSENSE_KEYMAP_ROW_TO_GPIO_ROW(row), time, reps);
}
...
uint16_t calibration_measure_all_valid_keys(uint8_t time, uint8_t reps, bool looking_for_all_zero)
{
    ...
    while (min < max)
    {
        ...
        for (col = 0; col < MATRIX_COLS; col++)
        {
            ...
            {
                if (keycode_at_keymap_location_raw(0, row, col) != KC_NO)
                {
                    valid_physical_rows |= (((matrix_row_t)1) << CAPSENSE_KEYMAP_ROW_TO_GPIO_ROW(row)); // convert keymap row to physical row
                }
            }
            uint8_t physical_col = CAPSENSE_KEYMAP_COL_TO_GPIO_COL(col);
            uint8_t i;
            for (i=0;i<reps;i++) {
                if (looking_for_all_zero)
                {
...
void calibration(void)
{
    ...
    for (col = 0; col < MATRIX_COLS; col++) {
        ...
        for (row = 0; row < MATRIX_CAPSENSE_ROWS; row++) {
            if (keycode_at_keymap_location_raw(0, row, col) != KC_NO) {
                uint16_t threshold = measure_middle(physical_col, CAPSENSE_KEYMAP_ROW_TO_GPIO_ROW(row), CAPSENSE_HARDCODED_SAMPLE_TIME, CAPSENSE_CAL_EACHKEY_REPS);
                uint8_t besti = 0;
...
void matrix_scan_raw(matrix_row_t current_matrix[]) {
    ...
    for (cal=0;cal<CAPSENSE_CAL_BINS;cal++) {
        ...
        for (col=0;col<MATRIX_COLS;col++) {
            uint8_t real_col = CAPSENSE_KEYMAP_COL_TO_GPIO_COL(col);
            ...
            for (row=0;row<MATRIX_CAPSENSE_ROWS;row++) {
                if (assigned_to_threshold[cal][row] & (((matrix_row_t)1) << col))
                {
                    ...
                    uint8_t physical_row = CAPSENSE_KEYMAP_ROW_TO_GPIO_ROW(row);
    ...
    for (col=0;col<MATRIX_COLS;col++)
    {
        ...
        for (row=0;row<MATRIX_CAPSENSE_ROWS;row++)
        {
            current_matrix[CAPSENSE_GPIO_ROW_TO_KEYMAP_ROW(row)] |= (((matrix_row_t)(d & 1)) << col);
            d >>= 1;
        }
    }
    #endif
